class DayZPlayerImplementFallDamage
{
	...
	const float		FD_MAX_BREAK_LEG_HEIGHT = 12;			//!< height where legs break most of the time
	...
	void HandleFallDamage(float pHeight)
	{
		if (GetGame().IsServer())
		{
			...
			float normalizedHeight = MiscGameplayFunctions.Normalize(pHeight, FD_MAX_BREAK_LEG_HEIGHT); //Normalize the height player fell from
			...
			dmg = (dmg * 100) * Math.RandomFloatInclusive(0.5, 2);
